...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    perfChartRef.current = new Chart(perfCanvasRef.current.getContext('2d'), {
      ...
      options: {
        ...
        scales: {
          y: { min: 0, max: 100, ticks: { stepSize: 25, color: '#6b7280' }, grid: { color: '#eef1f6' } },
  ...
  const pieTopics = useMemo(() => TOPICS.filter((t) => Object.prototype.hasOwnProperty.call(TIME_COLORS, t.id)), []);
  ...
  useEffect(() => {
    ...
    pieChartRef.current = new Chart(pieCanvasRef.current.getContext('2d'), {
      type: 'pie',
      data: { labels: pieTopics.map((t) => t.name), datasets: [{ data: values, backgroundColor: colors, borderColor: '#fff', borderWidth: 2 }] },
      options: {
        responsive: true,
        maintainAspectRatio: false,
        animation: { duration: 900, easing: 'easeOutQuart' },
  ...
  const pieValues = pieTopics.map((t) => Math.round(((data.learningTimeMinutes[t.id] || 0) / 60) * 10) / 10);
      ...
      <div className="dashboard-wrap page-shell-main">
          ...
          <div>
            ...
            <p className="subtitle">
              {user
                ...
                : <>You're browsing as a guest — progress is saved on this device only. <a href="/sign-in">Sign in</a> to save it to your account.</>}
            </p>
          </div>
          <button className="btn btn-continue" type="button" onClick={() => navigate(findContinueLearningUrl(data))}>
        ...
        <section className="row g-3 section-gap">
          <div className="col-6 col-lg-3">
            <div className="card-panel stat-card hoverable reveal-on-scroll">
              <div className="stat-top"><div className="stat-icon bg-blue"><i className="bi bi-journal-check"></i></div><span className="stat-badge">of {total}</span></div>
          ...
          <div className="col-6 col-lg-3">
            <div className="card-panel stat-card hoverable reveal-on-scroll">
              <div className="stat-top"><div className="stat-icon bg-orange"><i className="bi bi-percent"></i></div><span className="stat-badge">Average</span></div>
          ...
          <div className="col-6 col-lg-3">
            <div className="card-panel stat-card hoverable reveal-on-scroll">
              <div className="stat-top"><div className="stat-icon bg-pink"><i className="bi bi-fire"></i></div><span className="stat-badge">{streak === 1 ? 'Day' : 'Days'}</span></div>
          ...
          <div className="col-6 col-lg-3">
            <div className="card-panel stat-card hoverable reveal-on-scroll">
              <div className="stat-top"><div className="stat-icon bg-purple"><i className="bi bi-clock-history"></i></div><span className="stat-badge">Hours</span></div>
        ...
        <section className="row g-3 section-gap">
          <div className="col-lg-8">
            <div className="card-panel chart-card hoverable reveal-on-scroll">
              <div className="panel-title"><i className="bi bi-graph-up-arrow"></i> Performance Trend</div>
          ...
          <div className="col-lg-4">
            <div className="card-panel recommend-card reveal-on-scroll">
              <div className="recommend-eyebrow"><i className="bi bi-star-fill"></i> Recommended Major</div>
              ...
              <div className="recommend-progress"><span style={{ width: `${rec ? rec.percent : 0}%` }}></span></div>
              <div className="panel-subtitle" style={{ color: '#3c6a1e', marginBottom: '8px' }}>Based on:</div>
              <ul className="recommend-reasons">
                {rec
                  ? rec.reasons.map((r) => <li key={r}><i className="bi bi-check-circle-fill"></i><span>{r}</span></li>)
                  : <li><i className="bi bi-info-circle-fill"></i><span>Take a lesson quiz to get a personalized major recommendation.</span></li>}
              </ul>
              <button className="btn btn-retake" type="button" onClick={() => navigate('/quiz')}>Retake Quiz</button>
            </div>
          </div>
        </section>
        ...
        <section className="row g-3 section-gap">
                  ...
                  return (
                    <a key={topic.id} href={topic.lessonUrl} className="topic-row reveal-on-scroll">
                      <div className="topic-row-top">
                        ...
                        <span className="topic-count">{c} / {topic.total}</span>
                      </div>
          ...
          <div className="col-lg-4 d-flex flex-column gap-3">
            <div className="card-panel hoverable reveal-on-scroll">
              ...
              <div className="activity-list">
                {data.recentActivity.length === 0 ? (
                  <p className="panel-subtitle" style={{ margin: 0 }}>No activity yet — start a lesson to see it show up here.</p>
                ) : (
                  data.recentActivity.map((item, i) => {
                    const meta = ACTIVITY_ICON[item.type] || ACTIVITY_ICON.lesson;
                    const Tag = item.link ? 'a' : 'div';
                    return (
                      <Tag key={i} className="activity-item reveal-on-scroll" href={item.link || undefined}>
                        <div className={`activity-icon ${meta.cls}`}><i className={`bi ${meta.icon}`}></i></div>
                        <div>
            ...
            <div className="card-panel hoverable reveal-on-scroll">
              <div className="panel-title" style={{ marginBottom: '14px' }}>Quick Actions</div>
        ...
        <section className="row g-3">
          <div className="col-lg-8 mx-lg-auto">
            <div className="card-panel pie-card hoverable reveal-on-scroll">
              <div className="panel-title"><i className="bi bi-pie-chart-fill"></i> Time Distribution</div>
              <p className="panel-subtitle">Hours spent on each topic</p>
              <canvas ref={pieCanvasRef} height="220" style={{ display: pieHasData ? 'block' : 'none' }}></canvas>
              {!pieHasData && <p className="panel-subtitle" style={{ margin: 0 }}>No learning time logged yet — it'll show up here once you start a lesson.</p>}
              {pieHasData && (
                <div className="pie-legend">
                  {pieTopics.map((t, i) => (
                    <span key={t.id}><span className="dot" style={{ background: pieColors[i] }}></span>{t.name}: {pieValues[i]}h</span>
                  ))}
                </div>
              )}
            </div>
