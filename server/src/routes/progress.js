...
progressRouter.post('/lesson-complete', requireAuth, asyncHandler(async (req, res) => {
  const userId = req.session.userId;
  const { topicId, lessonUrl } = req.body || {};
  if (!topicId) return res.status(400).json({ error: 'topicId is required.' });

  const [topicRows] = await pool.query('SELECT total_lessons FROM topics WHERE id = ?', [topicId]);
...
progressRouter.post('/last-lesson', requireAuth, asyncHandler(async (req, res) => {
  const userId = req.session.userId;
  const { topicId, lessonUrl } = req.body || {};
  if (!topicId) return res.status(400).json({ error: 'topicId is required.' });
...
progressRouter.post('/quiz-result', requireAuth, asyncHandler(async (req, res) => {
  const userId = req.session.userId;
